...
| **A\*** | Algoritmo de búsqueda de camino mínimo que mejora a Dijkstra usando una **heurística** (p.ej. distancia al destino) para explorar antes las direcciones prometedoras. Nombrado por Enagás junto a Dijkstra para el trazado base (MVP 2). |
| **Matriz de condicionantes** | Clasificación de cada capa/criterio en **técnico** (pendiente, cruces), **ambiental** (Red Natura) o **administrativo** (uso de suelo, afección municipal). Puente entre las capas GIS y la superficie de coste (MVP 1). |
| **Validación de calidad de datos / detección de vacíos** | Comprobar cobertura, consistencia y huecos de cada capa sobre el AOI antes de combinarlas. Un vacío silencioso de datos también hace que los costes mientan. |
| **Backtesting (de trazados)** | Contrastar las alternativas generadas con un **ramal real existente**: ver si la ruta real aparece entre las generadas, explicar desviaciones vía pesos/condicionantes y ajustar el modelo de costes. Validación empírica del modelo (MVP 4/6). |
| **EV-500 / paquete tipo EV-500** | Formato de **pre-ingeniería de Enagás** para empaquetar la alternativa elegida: plano del trazado, exportable GIS, relación de cruces especiales, afección por términos municipales y narrativa justificativa. *Confirmar alcance exacto con Enagás* (MVP 5/8). |
| **Afección por términos municipales** | Km o impacto del trazado desglosado por cada municipio atravesado. Criterio administrativo del paquete EV-500. |
| **Versionado de escenarios / trazabilidad de decisiones** | Guardar de forma reproducible cada escenario (AOI, origen/destino, pesos) y el porqué de cada decisión de diseño. Requisito de industrialización (MVP 8). |
