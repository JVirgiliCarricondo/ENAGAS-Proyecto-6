...
> Resumen ejecutivo. Actualizar al cierre de cada sprint semanal. Última actualización: **2026-07-13**.
...
| Código | 🟢 En marcha | **MVP 1-4 operativos:** 6 capas alineadas (EPSG:25830 · ~30 m), 6 capas de coste, **motor LCP Dijkstra 8-conexo** (`src/trazados/ruta_pendiente.py`), 4 rutas por escenario con validación de diferenciación, métricas multicriterio y **app Streamlit completa** (asistente 3 pasos + informe PDF por escenario). Robustez S6-S7: catastro de la Sede por municipio (urbana/rústica, paquetes provinciales multiparte) + **catastro foral** (Navarra/País Vasco) automático, escenarios efímeros con **limpieza automática al abrir sesión** (A/B intocables), validación de origen/destino **en el mar** y re-descarga automática si cambian las coordenadas del AOI. |

## Hito actual

**Sprint 7 (13–17 jul): pulido + presentación final → demo 17-jul.** El pipeline end-to-end funciona (origen/destino → descarga → capas de coste → 4 rutas → métricas → comparativa + informe PDF). Foco de la semana: ensayo de la demo en vivo (escenarios A y B + un escenario nuevo con subida de catastro en directo), quick wins de la [auditoría del 13-jul](coordinacion/auditoria_2026-07-13.md) y presentación tipo mini-consultoría con los MVP 5-8 como hoja de ruta.
...
- **Ramal real para backtesting:** ⚠️ **no llegó para la demo** — no se recibió un ramal de H₂ real de Enagás, así que la validación de la presentación del 17-jul es interna (diferenciación + métricas). Queda como primera acción de continuidad si el proyecto sigue (MVP 4/6).
...
| S6 | 6-10 jul | Evaluación + backtesting con ramal real + robustez | MVP 4, 6 | ✅ (robustez de ingesta y catastro; backtesting pendiente de ramal de Enagás) |
| S7 | 13-17 jul | Pulido + presentación (+ EV-500/herramienta si da tiempo) | MVP 5/7 | 🟡 En curso |
