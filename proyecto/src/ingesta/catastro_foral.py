...
def servicios_para_aoi(aoi_25830: tuple[float, float, float, float]
    ...
    """Servicios forales cuyo extent provincial intersecta el AOI."""
...
def _fetch_wfs(endpoint: str, typename: str,
    ...
    """Una capa WFS por bbox. `None` si la petición o el parseo fallan."""
...
def _fetch_arcgis(base: str, layer_id: str,
    ...
    """Una capa de un MapServer ArcGIS por envelope, paginando resultados.

    `None` si la primera petición falla; GeoDataFrame (posiblemente vacío) si el
    servicio responde. El WFS de ArcGIS ignora el BBOX por KVP, por eso vamos por
    el REST /query con esriGeometryEnvelope.
    """
    if layer_id == "auto":
...
def _descargar_servicio(svc: ForalService,
                        bbox_25830: tuple[float, float, float, float],
                        log: logging.Logger) -> "gpd.GeoDataFrame | None":
    """Une todas las capas de parcelas de un territorio en un solo GeoDataFrame
    normalizado a [refcat, geometry] en EPSG:25830. `None` si la descarga falló."""
...
def _normalizar(gdf: "gpd.GeoDataFrame", ref_field: str | None
    ...
    """Deja solo [refcat, geometry] en EPSG:25830 y descarta no-polígonos."""
    if gdf.crs is None:
...
def descargar_catastro_foral(
    ...
    Best-effort: no lanza; devuelve un ForalResult por territorio consultado
    (lista vacía si el AOI no toca Navarra ni País Vasco).
    """
    if gpd is None:
