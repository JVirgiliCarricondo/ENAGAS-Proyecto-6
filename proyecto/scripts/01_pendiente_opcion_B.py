"""
Script 01 - Capa de coste P1: pendiente.

Convierte dem_aoi_{s}.tif en pendiente_{s}.tif (coste relativo [0, 1] + barreras).
Modelo de coste en modelo_coste.md (5.1) y contrato de salida del hito 2.
...
Patron comun de las capas de coste (hito 2):
  ...
  Paso 3 - Rasterizar: no aplica (el DEM ya esta sobre la rejilla común).
...
def validar_curva_mapeo() -> None:
    """Comprueba la curva normativa en puntos de control."""
    casos = [
        (0.0, 0.0),
        (5.0, 0.0),
        (10.0, 0.3),
    ...
    for pend, esperado in casos:
        resultado = mapeo_coste_pendiente(np.array([pend], dtype=np.float32))[0]
        if np.isinf(esperado):
            assert np.isinf(resultado)
        else:
            assert abs(float(resultado) - esperado) < 1e-5
...
def procesar_escenario(s: str) -> Path:
    ...
    with rasterio.open(dem_path) as ref:
        ...
        if ref.crs is None or ref.crs.to_epsg() != 25830:
            raise ValueError(f"CRS incorrecto en {dem_path}: se requiere EPSG:25830")
...
def main() -> None:
    ...
    print("Script 01 completado: capa de coste 'pendiente' generada (A y B).")
